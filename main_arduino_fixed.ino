...
const int pin_mosfet_pump = 5;    // Pump relay/MOSFET (Pin 5 supports analogWrite)
...
int moist_min = 30;       // Lower limit: Full pump speed when soil drops below this (%)
int moist_max = 70;       // Upper limit: Pump off when soil reaches this (%)
int pump_power = 255;     // (Legacy/reference) Max pump PWM ceiling — not used in auto speed calc
...
// >>> ADDED: Stores the calculated PWM value (0-255) sent to the pump motor
int pump_pwm = 0;
...
void operatingScreen(){
  ...
  // ============================================================
  // >>> ADDED: Step B — Proportional Pump Speed Calculation
  //
  // The pump PWM is determined by 3 moisture zones:
  //
  //   Zone 1 | moisture < moist_min (< 30%)
  //           → Soil is DRY  → Run pump at FULL speed (PWM = 255)
  //
  //   Zone 2 | moist_min <= moisture <= moist_max (30% – 70%)
  //           → Soil is PARTIAL → Gradually REDUCE speed as moisture rises
  //             PWM is linearly mapped: 255 at moist_min → 0 at moist_max
  //
  //   Zone 3 | moisture > moist_max (> 70%)
  //           → Soil is WET  → Turn pump OFF (PWM = 0)
  // ============================================================
  if (current_moist < moist_min) {
    ...
    // Zone 2: Proportional speed — higher moisture = lower PWM
    pump_pwm = map((int)current_moist, moist_min, moist_max, 255, 0);
    pump_pwm = constrain(pump_pwm, 0, 255); // Safety clamp
    is_pumping = true;
  }
  else {
    // Zone 3: Pump off
  ...
  // >>> CHANGED: Use analogWrite instead of digitalWrite so the motor
  //     receives the calculated variable speed PWM signal (0–255).
  //     NOTE: Pin 5 supports analogWrite — Servo.h only disables
  //     PWM on pins 9 & 10, so this is safe.
  ...
  // >>> CHANGED: Second row now shows pump PWM value instead of just ON/OFF
  //     so the operator can see the current motor speed level.
  lcd.setCursor(0, 0);
  ...
  lcd.setCursor(9, 1);
  // >>> ADDED: Show live PWM value (e.g. "PWM:128") so speed is visible
  ...
  if (button_clicked) {
    // >>> CHANGED: analogWrite(0) instead of digitalWrite LOW to properly
    //     drive the MOSFET gate to zero when using PWM mode.
    ...
    is_pumping = false;
    is_refilling = false;
    pump_pwm = 0; // >>> ADDED: Reset PWM tracker on exit
